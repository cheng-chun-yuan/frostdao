...
`protocol::SigningAttemptCollector` tracks one active signing attempt after decryption. It accepts only signing inputs for the configured wallet, session, attempt ID, signer set, and sighash fingerprint, requires a nonce before accepting a signature share from the same signer, and reports when nonce/share thresholds are met. This is the coordinator boundary before full ROAST retry and blame logic; Nostr remains the relay/mailbox layer.
