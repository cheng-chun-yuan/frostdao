...
`tx_proposal` must include `unsigned_tx` plus a `review` object with `network`, `source_path`, `from_address`, `to_address`, `amount_sats`, `fee_rate_sats_vb`, and `sighash_fingerprint`. Signers should compare these fields across devices before consenting. The unsigned transaction is public proposal metadata, not a signed transaction, nonce, share, or secret. The TUI outbound proposal path builds proposals through the DKG transaction builder so the session, unsigned transaction, and fingerprint come from a real unsigned transaction and BIP341 sighash. The TUI runtime accepts inbound proposals only when the proposal's `proposer_index` matches the envelope `from`, the proposer is inside the active room range, amount and fee rate are nonzero, `unsigned_tx` is non-empty and decodes as a Bitcoin transaction, source and recipient addresses parse for the selected network, review amount/fee/recipient fields match the proposal payload, review network matches the selected TUI network, and the review fingerprint matches the proposal sighash.
...
Session-scoped signing messages must carry a non-empty envelope `session`. Wallet-scoped signing messages must also carry a non-empty envelope `wallet`. The TUI runtime rejects `tx_proposal`, `tx_consent`, `signing_nonce_encrypted`, `signing_share_encrypted`, and `tx_broadcast` messages without an explicit session; consent envelope sessions must match the `proposal_session` payload. TUI outbound encrypted nonce/share messages require a wallet, session, non-empty ciphertext, and a recipient party inside the active room that is not the sender. Encrypted signing nonce/share payloads must bind `party_index` to the envelope `from`, bind `to_index` to the direct envelope `to`, and come from a party in the active room range. In relay mode, the TUI derives the NIP-44 conversation key from the local relay session key and the peer's joined Nostr pubkey, encrypts outbound typed nonce/share plaintext, decrypts inbound typed nonce/share plaintext, and maps only validated plaintext into coordinator input. Inbound share ciphertexts are ignored until a nonce ciphertext from the same session and sender has already been accepted. When the TUI enters share collection, it starts a session-keyed `protocol::SigningCoordinator`, replays already accepted session nonces, and advances to combining only after the coordinator reaches share threshold. Pending proposals are labeled by wallet and the proposal picker only shows proposals for the selected wallet. TUI outbound and inbound `tx_broadcast` announcements are accepted only when the wallet, session, sender room range, selected network, raw transaction hex, and txid all match. Accepted inbound TUI signing messages append metadata-only audit entries; ciphertext, raw transactions, full sighashes, nonces, shares, and secret material are excluded.

Use `encrypt_signing_nonce_plaintext` and `encrypt_signing_share_plaintext` to encode typed signing plaintexts as NIP-44 ciphertext before publishing. After receiving `signing_nonce_encrypted` or `signing_share_encrypted`, use the matching decrypt helper so the plaintext is decoded and validated against the accepted envelope before it is converted into protocol-level coordinator input. Both schemas bind `wallet`, `session`, `attempt_id`, `signer_set`, `party_index`, `to_index`, and `sighash_fingerprint` to the accepted envelope and payload; nonce plaintexts additionally carry `public_nonce`, and share plaintexts carry `signature_share`. Receivers must reject decrypted plaintext whose wallet, session, sender, recipient, attempt, signer set, or fingerprint context does not match the active signing attempt.

`protocol::SigningCoordinator` tracks one active signing attempt after decryption. It validates the selected TSS or HTSS signer policy, accepts only signing inputs for the configured wallet, session, attempt ID, signer set, and sighash fingerprint, requires nonce threshold before accepting signature shares, requires each signer share to follow that signer nonce, and reports when the attempt is ready to combine. This is the coordinator boundary before full ROAST retry and blame logic; Nostr remains the relay/mailbox layer.
