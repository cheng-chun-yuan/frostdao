...
The TUI room screen creates a `NostrRoomRuntime` when joining a room. The current TUI runtime uses deterministic in-memory transport for local testnet/demo flows, while still validating versioned envelopes and persisting replay-cache IDs under `.frost_state/nostr_replay/`. Room joins are accepted only when the payload party, threshold, party count, scheme, and rank shape match the active room. TUI transaction proposals require a non-empty recipient address valid for the selected network, a nonzero amount, and a loaded wallet with a known source address before the proposal and consent actions publish versioned `tx_proposal` and `tx_consent` messages through this runtime. Runtime polling also ingests incoming `tx_proposal` messages into the pending proposal list only when proposer identity, active room range, network, addresses, amount, fee, and sighash fingerprint match the review payload; incoming `tx_consent` messages are applied to the proposer consent state only when the signer is inside the active room and the reviewed fingerprint matches the active proposal. Encrypted `signing_nonce_encrypted` and `signing_share_encrypted` messages can be published through the same runtime and are ingested into session-keyed TUI inboxes only when the ciphertext payload party and recipient match the envelope sender and local party. Session-scoped signing messages without an explicit envelope `session` are ignored, and pending proposals are filtered by their envelope `wallet` before display. `tx_broadcast` announcements are also published and ingested through the runtime; inbound broadcasts complete an active session only when wallet, session, sender room range, selected network, raw transaction hex, and txid match. Audit records include only broadcast metadata such as txid and network, not raw transaction hex. The TUI does not simulate final broadcast completion: it waits for a real matching `tx_broadcast` announcement, so use the CLI transaction broadcast flow until relay-backed TUI signing is fully wired.
