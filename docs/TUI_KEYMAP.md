...
Wallet Details shows the active network, policy, address scope, and `UTXO API` before the wallet address, so balance fetch and send readiness use the same network facts as Home and Chain Select. Home send readiness means the source address and UTXO API are available to fetch UTXOs; transaction review still requires confirmed UTXOs and enough confirmed balance. If the selected wallet has no address for the active network, Home and Wallet Details name the missing network address and block send, copy, and QR expectations until the user selects a wallet/network with an address. The QR popup labels the address as a public receive address and reminds the operator to verify the network before funding.
...
The selector shows the selected network policy before confirmation: testnet4/testnet3/signet use remote mempool.space UTXOs, regtest uses the local Esplora/mempool API from `FROSTDAO_REGTEST_MEMPOOL_API`, and mainnet requires explicit real-funds opt-in. It also shows the exact `UTXO API` endpoint or missing regtest configuration that balance, send, and Nostr proposal flows will depend on. Confirming a different network clears pending send, reshare, and volatile Nostr ceremony state; confirming the current network leaves those drafts unchanged. Home shows the same `UTXO API` line for the active network. Home Nostr readiness says to configure and join a room until a room is active; after joining, it distinguishes local rehearsal rooms from relay signing rooms.
...
The configure screen labels this as a multi-device signing room. The room info line shows room ID, party index, threshold, scheme, rank, and transport. Join validation honors the configured scheme and requires `Rank` only in HTSS mode; room join metadata is public, while signing nonce/share payloads are encrypted. The configure status line shows `Blocked` until the room ID, party index, threshold, and party count form a valid room.
When all parties have joined, local simulation labels the room as ready for a
local rehearsal. Relay transport labels the room as ready for signing only.
