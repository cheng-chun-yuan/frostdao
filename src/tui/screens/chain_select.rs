...
pub(crate) fn chain_select_policy_lines(network: NetworkSelection) -> Vec<Line<'static>> {
    ...
    vec![
        ...
        Line::from(Span::styled(
            "Confirming a different network clears pending send, reshare, and volatile Nostr ceremony state.",
            Style::default().fg(Color::DarkGray),
        )),
    ]
}
...
mod tests {
    ...
    fn chain_select_policy_lines_show_remote_testnet_sources() {
        ...
        assert!(rendered.contains("Confirming a different network clears pending send, reshare"));
    }
}
